...
class SectionedMultiSelect extends PureComponent {
  ...
  static propTypes = {
    ...
    renderSelectTextComponent: PropTypes.func,
  ...
  _getSelectLabel = () => {
    ...
    if (!selectedItems || selectedItems.length === 0) {
      ...
      let foundItem = this._findItem(item)

      if (foundItem) {
        this.selectedCache[item] = foundItem
      } else {
        foundItem = this.selectedCache[item]
      }

      label = this.getProp(foundItem, displayKey) || this.getProp(this.selectedCache[item], displayKey) || selectText
    }
    if (renderSelectText && customSelect && typeof customSelect === 'string') {
      label = customSelect
    }

    if (renderSelectTextComponent) {
      return renderSelectTextComponent(label)
    }
  ...
  _toggleItem = (item, hasChildren) => {
    ...
    this.selectedCache[item[uniqueKey]] = item
    if (single) {
  ...
  _displaySelectedItems = () => {
    ...
    return selectedItems.map((singleSelectedItem) => {
      let item = this._findItem(singleSelectedItem)
      if (item) {
        this.selectedCache[singleSelectedItem] = item
      } else {
        item = this.selectedCache[singleSelectedItem]
      }
  ...
  render() {
    ...
    const renderItems = searchTerm && !filterAsync ? this._filterItems(searchTerm.trim()) : items
              ...
              <View style={[{ flexDirection: 'row', paddingVertical: 5 }, styles.searchBar]}>
                ...
                <TextInput
                  ...
                  onChangeText={searchTerm => {
                    filterAsync && filterAsync(searchTerm)
                    this.setState({ searchTerm })
                  }}
